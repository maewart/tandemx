...
print(tiledir)
dem_fn = glob.glob(os.path.join(tiledir, 'DEM/*DEM_projected.tif'))[0]
print(dem_fn)
...
err_fn = glob.glob(os.path.join(tiledir, 'AUXFILES/*HEM_projected.tif'))[0]
err = iolib.fn_getma(err_fn)
max_err_multi = 1.5
mask = np.logical_or(mask, (err.data > max_err_multi))

#Water mask
wam_fn = glob.glob(os.path.join(tiledir, 'AUXFILES/*WAM_projected.tif'))[0]
...
#Consistency mask
com_fn = glob.glob(os.path.join(tiledir, 'AUXFILES/*COM_projected.tif'))[0]
